...
本仓库现在包含一个最小可运行的前后端分离骨架：

- `backend/`：Django + Django REST Framework API，包含健康检查、OpenAPI schema 和 Swagger UI。
- `frontend/`：Vue 3 + TypeScript + Vite + Pinia + Element Plus 首页，默认调用 `/api/health/`。
- `docker-compose.yml`：本地 PostgreSQL + pgvector 与 Redis 服务。
- `.github/workflows/ci.yml`：后端 lint/Django check 与前端 build CI。

详细启动步骤见 [`docs/development.md`](docs/development.md)。
